...
branch_and_reduce_algorithm::branch_and_reduce_algorithm(
    graph_access &G, const MISConfig &config, bool called_from_fold)
    : config(config), global_status(G), set_1(global_status.n),
      set_2(global_status.n), double_set(global_status.n * 2),
      buffers(2, sized_vector<NodeID>(global_status.n)) {

  if (called_from_fold) {
    global_status.reductions =
        make_reduction_vector<neighborhood_reduction, fold2_reduction,
                              clique_reduction, domination_reduction,
                              twin_reduction>(global_status.n);
  } else if (config.reduction_style == MISConfig::Reduction_Style::DENSE) {
    global_status.reductions =
        make_reduction_vector<neighborhood_reduction, fold2_reduction,
                              clique_reduction, domination_reduction,
                              twin_reduction, generalized_fold_reduction>(
            global_status.n);
  } else {
    // MISConfig::Reduction_Style::NORMAL
    global_status.reductions = make_reduction_vector<
        neighborhood_reduction, fold2_reduction, clique_reduction,
        domination_reduction, twin_reduction, clique_neighborhood_reduction,
        critical_set_reduction, generalized_fold_reduction>(global_status.n);
  }
    ...
    if (this->config.reduction_style == MISConfig::Reduction_Style::DENSE) {
      status.reductions =
          make_reduction_vector<neighborhood_reduction, fold2_reduction,
                                clique_reduction, domination_reduction,
                                twin_reduction,
                                clique_neighborhood_reduction_fast>(status.n);
    } else {
      // MISConfig::Reduction_Style::NORMAL
      if (called_from_fold) {
        status.reductions =
            make_reduction_vector<neighborhood_reduction, fold2_reduction,
                                  clique_reduction, domination_reduction,
                                  twin_reduction, clique_neighborhood_reduction,
...
void branch_and_reduce_algorithm::set(NodeID node, IS_status mis_status,
                                      bool push_modified) {
...
void branch_and_reduce_algorithm::flip_include_exclude(NodeID node) {
  ...
  if (status.node_status[node] == IS_status::included) {
    ...
    for (auto neighbor : status.graph[node]) {
      ...
      // TODO: try calling "add_next_level_neighborhood" in second loop after
      // this one
      add_next_level_neighborhood(neighbor);
    }
  }
...
void branch_and_reduce_algorithm::greedy_initial_is(
    graph_access &G, sized_vector<NodeID> &tmp_buffer) {
...
void branch_and_reduce_algorithm::compute_ils_pruning_bound() {
  ...
  config_cpy.time_limit =
      config_cpy.time_limit * status.n / total_ils_node_count / 100;

  best_weight = status.reduction_offset + status.is_weight +
                run_ils(config_cpy, *local_graph, buffers[0], 1000);

  cout_handler::enable_cout();
  std::cout << (get_current_is_weight() + best_weight) << " [" << t.elapsed()
            << "]" << std::endl;
}
...
NodeWeight branch_and_reduce_algorithm::compute_cover_pruning_bound() {
  ...
  std::sort(nodes.begin(), nodes.end(), [&](NodeID lhs, NodeID rhs) {
    return (
        status.weights[lhs] > status.weights[rhs] ||
  ...
  for (NodeID node : nodes) {
    ...
    for (NodeID neighbor : status.graph[node]) {
      if (status.node_status[neighbor] == IS_status::not_set) {
        ...
        if (covered[w]) {
          ...
          if (neigh_clique_sizes[c] == clique_sizes[c] &&
              clique_weight[c] > clique_weight[heaviest_clique])
            heaviest_clique = c;
        }
      }
    }
    ...
    clique_weight[heaviest_clique] =
        std::max(clique_weight[heaviest_clique], status.weights[node]);
...
size_t branch_and_reduce_algorithm::run_ils(const MISConfig &config,
                                            graph_access &G,
                                            sized_vector<NodeID> &tmp_buffer,
                                            size_t max_swaps) {
...
void branch_and_reduce_algorithm::init_reduction_step() {
  if (!status.reductions[active_reduction_index]->has_run) {
    status.reductions[active_reduction_index]->marker.fill_current_ascending(
        status.n);
...
void branch_and_reduce_algorithm::add_next_level_nodes(
...
void branch_and_reduce_algorithm::add_next_level_nodes(
    const sized_vector<NodeID> &nodes) {
  for (auto node : nodes) {
    add_next_level_node(node);
  }
}
...
void branch_and_reduce_algorithm::add_next_level_neighborhood(NodeID node) {
  // node has been excluded in mis -> neighboring vertices are interesting for
  // next round of reduction
  for (auto neighbor : status.graph[node]) {
    add_next_level_node(neighbor);
  }
}

void branch_and_reduce_algorithm::add_next_level_neighborhood(
    const std::vector<NodeID> &nodes) {
  for (auto node : nodes) {
    add_next_level_neighborhood(node);
  }
...
void branch_and_reduce_algorithm::reduce_graph_internal() {
  ...
  do {
    // std::cout << "\ncurrent weight: " << status.is_weight << "  weight
    // offset: " << status.reduction_offset << "  remaining nodes: " <<
    // status.remaining_nodes << std::endl;
    progress = false;

    for (auto &reduction : status.reductions) {
      active_reduction_index =
          local_reduction_map[reduction->get_reduction_type()];
  ...
  // std::cout << "\ncurrent weight: " << status.is_weight
  //          << "  weight offset: " << status.reduction_offset
...
bool branch_and_reduce_algorithm::branch_reduce_recursive() {
  ...
  size_t comp_count = strongly_connected_components().strong_components(
      recursive_graph, recursive_comp_map);
  ...
  for (size_t i = 0; i < comp_count; i++) {
    ...
    forall_nodes(G, node) {
      recursive_graph.setPartitionIndex(recursive_local_mapping[node],
                                        G.getPartitionIndex(node));
    }
    endfor
  }
...
void branch_and_reduce_algorithm::branch_reduce_single_component() {
  ...
  if (status.n == 0) {
    ...
    std::cout << (get_current_is_weight() + status.is_weight +
                  status.reduction_offset)
              << " [" << t.elapsed() << "]" << std::endl;
    return;
  }
  ...
  while (i < status.n) {
    ...
    bool improvement_possible = status.is_weight + status.reduction_offset +
                                    compute_cover_pruning_bound() >
                                best_weight;

    if (improvement_possible &&
        status.node_status[branch_node] == IS_status::not_set) {
...
bool branch_and_reduce_algorithm::run_branch_reduce() {
  ...
  std::cout << "%reduction_offset "
            << global_status.is_weight + global_status.reduction_offset << "\n";
  ...
  // TODO print kernel global_graph
  graph_io::writeGraphWeighted(global_graph, "kernel.graph");
  return;

  std::vector<int> comp_map(global_status.remaining_nodes, 0);
  size_t comp_count =
      strongly_connected_components().strong_components(global_graph, comp_map);
  ...
  std::sort(comp_idx.begin(), comp_idx.end(),
            [&comp_size](const size_t lhs, const size_t rhs) {
  ...
  for (size_t i : comp_idx) {
    ...
    // std::cout << "%connected component " << i << ":  " << comp_size[i] <<
    // std::endl;
    ...
    for (size_t node = 0; node < status.n; node++) {
      // if (status.node_status[node] == IS_status::not_set ||
      // status.node_status[node] == IS_status::folded) { 	std::cerr <<
      // "error: node is not_set / folded after restore" << std::endl; 	exit(1);
      // }

      global_status.node_status[global_mapping[local_mapping[node]]] =
...
void branch_and_reduce_algorithm::update_best_solution() {
  ...
  if (current_weight > best_weight) {
    ...
    std::cout << (get_current_is_weight() + current_weight) << " ["
              << t.elapsed() << "]" << std::endl;
  }
}
...
void branch_and_reduce_algorithm::reverse_branching() {
  ...
  while (!status.modified_queue.empty() &&
         !is_token(status.modified_queue.back())) {
...
void branch_and_reduce_algorithm::build_graph_access(
    graph_access &G, std::vector<NodeID> &reverse_mapping) const {
  ...
  for (size_t i = 0; i < status.remaining_nodes; ++i) {
    ...
    std::sort(std::begin(adjncy) + xadj[i],
              std::begin(adjncy) + adjncy_counter);
  }
...
void branch_and_reduce_algorithm::build_induced_neighborhood_subgraph(
    graph_access &G, NodeID source_node) {
  buffers[0].clear();
  set_1.clear();
...
void branch_and_reduce_algorithm::build_induced_subgraph(
    graph_access &G, const sized_vector<NodeID> &nodes,
    const fast_set &nodes_set, sized_vector<NodeID> &reverse_mapping) {
  size_t edge_count = 0;
...
void branch_and_reduce_algorithm::reverse_reduction(
...
void branch_and_reduce_algorithm::apply_branch_reduce_solution(
    graph_access &G) {
  forall_nodes(G, node) {
    if (status.node_status[node] == IS_status::included) {
