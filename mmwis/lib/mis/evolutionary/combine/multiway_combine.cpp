...
multiway_combine::multiway_combine(const MISConfig& config) : config(config), local(config) {
...
void multiway_combine::combine(MISConfig & config, graph_access & G, separator_pool *pool, population_mis & pop, individuum_mis & out) {
    ...
    if (config.optimize_candidates) local.direct_improvement(G, node_candidates);
    else local.make_maximal(G);

    // Generate solution
    NodeID *solution = new NodeID[G.number_of_nodes()];
    NodeWeight solution_weight = pop.create_solution(G, solution);
    out.solution = solution;
    out.solution_weight = solution_weight;
...
void multiway_combine::build_combined_solution_vc(MISConfig & config, graph_access & G, std::vector<individuum_mis> & best_parents, std::vector<NodeID> & candidates) {
    ...
    forall_nodes(G, node) {
        unsigned int partition_index = best_parents[G.getPartitionIndex(node)].solution[node];

        if (partition_index == 0) G.setPartitionIndex(node, 0);
        else if (partition_index == 1) G.setPartitionIndex(node, 1);
...
void multiway_combine::build_combined_solution_ns(MISConfig & config, graph_access & G, std::vector<individuum_mis> & best_parents, std::vector<NodeID> & candidates) {
    ...
    forall_nodes(G, node) {
        ...
        if (G.getPartitionIndex(node) == G.get_partition_count()) partition_index = 2;
        else partition_index = best_parents[G.getPartitionIndex(node)].solution[node];

        if (partition_index == 0) G.setPartitionIndex(node, 0);
        else if (partition_index == 1) G.setPartitionIndex(node, 1);
        else if (partition_index == 2) G.setPartitionIndex(node, 2);
