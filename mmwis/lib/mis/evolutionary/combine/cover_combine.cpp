...
#include "max_flow/bipartite_weighted_vertex_cover.h"
#include "data_structure/priority_queues/bucket_array.h"

using namespace mmwis;

cover_combine::cover_combine(const MISConfig& config) : config(config), local(config) {}
...
void cover_combine::combine(MISConfig & config, graph_access & G, separator_pool *pool, individuum_mis & first, individuum_mis & second, individuum_mis & out_first, individuum_mis & out_second) {
    ...
    if (config.use_max_flow) {
        build_max_flow_cover(config, G, cover_graph);
    } else {
    ...
    first_out_complement.solution_weight = first_vertex_cover_weight;
    ...
    second_out_complement.solution_weight = second_vertex_cover_weight;
