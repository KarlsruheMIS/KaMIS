...
void local_search::make_maximal(graph_access& G) {
        if (sort_freenodes) {
                std::vector<NodeID> free_nodes(perm.get_free_size());
                for (size_t i = 0; i < perm.get_free_size(); i++) {
                        free_nodes[i] = perm.get_free_node(i);
                }

                sort_by_weight(G, free_nodes.begin(), free_nodes.end());
                ...
                forall_out_edges (G, target_edge, target) {
                        ...
                        if (perm.is_solution_node(candidate)) {
                                if (!candidates.contains(candidate))
                                        add_candidate(candidate, G);
                                else
                                        update_candidate(candidate, target, G);
...
local_search::Swap_1_2 local_search::find_best_swap(NodeID node, graph_access& G) {
        ...
        for (size_t i = 0; i < onetight_size && 2 * G.getNodeWeight(onetight[i]) > best_gain; i++) {
                ...
                for (size_t j = i + 1; j < onetight_size && G.getNodeWeight(onetight[j]) + first_gain > best_gain;
                     j++) {
                        // check if 1-tight neighbors are connected
                        bool connected = false;
...
void local_search::sort_by_weight(graph_access& G, std::vector<NodeID>::iterator begin,
