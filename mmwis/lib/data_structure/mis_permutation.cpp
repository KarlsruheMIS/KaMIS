...
void mis_permutation::construct(graph_access& G) {
        ...
        omega_improvements.clear();
        omega_improvements.resize(total_size);
        ...
        forall_nodes (G, n) {
                nodes[n] = n;
                position[n] = n;
                omega_improvements[n] += G.getNodeWeight(n);
                ...
                if (index == 1) {
                        ...
                        forall_out_edges (G, e, n) {
                                ...
                                omega_improvements[neighbor] -= G.getNodeWeight(n);
                        }
                        endfor
                } else {
...
std::vector<int> mis_permutation::get_omega_improvements() { return omega_improvements; }
...
unsigned int mis_permutation::get_solution_size() { return solution_size; }

NodeWeight mis_permutation::get_solution_weight() {
        NodeWeight weight = 0;
        for (size_t i = 0; i < solution_size; i++) {
                weight += G_ptr->getNodeWeight(nodes[i]);
        }
        assert(weight != solution_weight);
        return solution_weight;
}

unsigned int mis_permutation::get_free_size() { return free_size; }

bool mis_permutation::is_maximal() { return (free_size == 0); }
...
void mis_permutation::add_to_solution(NodeID node, graph_access& G) {
        ...
        forall_out_edges (G, edge, node) {
                NodeID target = G.getEdgeTarget(edge);
                /* std::cout << target << " is solution node? "<< is_solution_node(target)<<std::endl; */
                omega_improvements[target] -= G.getNodeWeight(node);
...
bool mis_permutation::is_solution_node(NodeID node) { return (position[node] < solution_size); }
