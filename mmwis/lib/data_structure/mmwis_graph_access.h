/******************************************************************************
 * mmwis graph_access.h
 *
 *****************************************************************************/

#ifndef MMWIS_GRAPH_ACCESS_EFRXO4X2
#define MMWIS_GRAPH_ACCESS_EFRXO4X2
...
#include "modKaHIPFiles/lib/data_structure/graph_access.h"
namespace mmwis {
...
public:
        graph_access()
        {
                ...
                m_max_degree = 0;
                m_max_weight_computed = false;
                m_max_weight = 0;
                graphref = std::make_shared<basicGraph>();
        ...
        mmwis::graph_access &operator=(const ::graph_access &other) {
                graphref = std::make_shared<basicGraph>(*other.graphref);
                m_max_degree_computed = other.m_max_degree_computed;
                m_max_weight_computed = false;
                m_partition_count = other.m_partition_count;
                m_max_degree = other.m_max_degree;
                m_max_weight = 0;
                m_separator_block_ID = other.m_separator_block_ID;
                m_second_partition_index = other.m_second_partition_index;
                return *this;
        }
...
inline EdgeWeight graph_access::getMaxDegree()
{
        if (!m_max_degree_computed)
        {
                ...
                forall_nodes(ref, node)
                {
                        ...
                        endfor if (cur_degree > m_max_degree)
                        {
